<template>
	<div>
      ...
      <!-- start published at -->
			...
			<v-col cols="12 d-flex" v-if="the_invoice.mailedto">
				<v-icon class="d-none d-sm-flex text--black mr-8">mdi mdi-email-outline</v-icon>
				<v-card class="px-5 flex-grow-1" outlined tile>
					<v-row>
						<v-col cols="12" md="6">
							<h4 class="text--primary d-inline">Emailed to </h4>
							<span>{{ the_invoice.mailedto }} {{ the_invoice.createdate }}</span>
						</v-col>

						<v-col cols="12" md="6">
							<h4 class="text--primary d-inline">Invoice sent as</h4>
							<span>{{ the_invoice.leveransmetod }}</span>
						</v-col>
					</v-row>
				</v-card>
			</v-col>
