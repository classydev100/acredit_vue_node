					...
					<v-row align-content="center" v-if="!signedUp">
						...
						<v-col cols="12" md="6">
							<v-text-field
								...
								:rules="[rules.passRules, rules.required]"
							/>
						</v-col>
						...
						<v-col cols="12" md="6">
							<v-text-field
								...
								dense
								outlined
								:rules="[rules.confirmPasswordRules, rules.required]"
						...
						<v-col cols="12">
							...
							<v-btn
								color="primary"
								@click="userRegisterBankId"
								class="mb-2"
							>{{ $t('loginForm.signUpBankId') }}</v-btn>
							<br />
			...
			<div class="form-container sign-in-container">
				<v-form v-model="valid" ref="form">
					<v-row align-content="center">
						...
						<v-col cols="12">
              <nuxt-link :to='`${$t("link")}resetPassword/useremail/`'>{{ $t('loginForm.forgot') }}</nuxt-link>
						</v-col>
						<v-col cols="12">
							<v-btn
								color="primary"
...
export default {
	...
	data() {
		return {
			...
			snackbar: false,
			showPassword: false,
			text: this.$t('hints.wrongMailOrPass'),
			rules: {
				emailRules: [
					v => !!v || this.$t('hints.required'),
					v => /.+@.+\..+/.test(v) || this.$t('hints.mailNotValid')
				],
				passRules: [v => !!v || this.$t('hints.required')],
				required: v => !!v || this.$t('hints.required'),
			...
			confirmBankValue: "this is the test",
			snackbar: false,
			timeout: 5000,
			color: "",
			showPass: false,
	...
	methods: {
		...
		async userRegister() {
			try {
        this.signUpUserInfo.email = this.signUpUserInfo.email.toLowerCase();

				await this.$axios
					.post("users", this.signUpUserInfo)
					.then(res => {
						this.text = this.$t('hints.successReg');
						this.color = "success";
						this.snackbar = true;
						this.signedUp = true;
				...
				this.text = this.$t('hints.noInternet');
				this.color = "error";
				this.snackbar = true;
			}
		},
		...
		//signup with bankid
		userRegisterBankId() {
			window.open('https://acredit-test.criipto.id/oauth2/authorize?response_type=id_token&client_id=urn:my:application:identifier:3502&acr_values=urn:grn:authn:se:bankid:another-device&redirect_uri=http://localhost:3000/banksignup&scope=openid','MyWindow','width=600,height=300');
		},
		// Login with bankid
		bankIdLogin() {
			window.open('https://acredit-test.criipto.id/oauth2/authorize?response_type=id_token&client_id=urn:my:application:identifier:3502&acr_values=urn:grn:authn:se:bankid:another-device&redirect_uri=http://localhost:3000/banksignup&scope=openid&state=etats','MyWindow','width=600,height=300');
		},
