		...
		<v-row>
			...
			<v-col cols="12" class="d-flex">
				...
				<v-menu offset-y v-if="draft">
					...
					<v-list>
						<v-list-item v-if="draft" @click="deleteDraft">{{ $t('delete') }}</v-list-item>
					</v-list>
				</v-menu>
			</v-col>
					...
					<v-dialog v-model="dialog" persistent scrollable max-width="600px">
						...
						<template v-slot:activator="{ on }">
							<div style="border: 1px dashed #aaa;" class="pa-3 mt-3">
								<v-btn
									dark
									class="my-3 my-md-auto flex-grow-1 width"
									v-on="on"
								>{{ $t('newInvoice.product.modal.newRow') }}</v-btn>

								<!-- Start Add Text -->
								<v-dialog v-model="addTextDialog" v-if="!hideAddText" width="500">
									<template v-slot:activator="{ on }">
										<v-btn
											color="red lighten-2 width"
											dark
											v-on="on"
										>{{ $t('newInvoice.product.modal.addText') }}</v-btn>
									</template>

									<v-card>
										<v-card-title>
											<h4>{{ $t('newInvoice.product.modal.addText') }}</h4>
											<v-spacer />
										...
										<v-card-actions class="grey lighten-3 pa-5">
											...
											<v-btn text @click="addTextDialog = false">{{ $t('cancle') }}</v-btn>
											<v-spacer />
											<v-btn
												color="primary"
												:disabled="!addTextVal"
									...
									<v-row v-if="!createNewModal">
										...
										<v-row v-if="selection_value != null">
											...
											<v-col cols="12">
												...
												>{{ (Number(selection_value.number) * Number(selection_value.pris_enhet).toFixed(2) ) || "0,00" }} Kr</p>
											</v-col>
											<!-- End Amount ex VAT -->
										</v-row>
										<!-- End Standard Edit -->
									...
									<v-form ref="form" v-else v-model="valid">
										...
										<v-row>
											...
											<v-col cols="12">
												...
												<p
													class="font-weight-medium"
												>{{ (selection_value.pris_enhet * selection_value.number).toFixed(2) || "0,00" }} Kr</p>
											</v-col>
											<!-- End Amount ex VAT -->
										</v-row>
							...
							<v-row>
								<v-col class="align-center d-flex">
									<div class="d-inline-block">
										...
										<b>{{ calculations.RoundedSumState ? Math.round(Number(calculations.totalSumToPay).toFixed(2)) : Number(calculations.totalSumToPay).toFixed(2) }} kr</b>
									</div>
	...
	methods: {
				...
				await this.$axios
					...
					.then(res => {
						...
						this.draggableItems.map(x => {
							x.total = (x.number * x.pris_enhet).toFixed(2);
						});
		...
		addToInvoice() {
			...
			if (this.editedIndex > -1) {
				...
				clonedObj.total = (clonedObj.number * clonedObj.pris_enhet).toFixed(2);
		...
		doCalculations(arr) {
			...
			arr.map(x => {
				...
				calcs.totalSumToPay =
					(calcs.amountExVAT + calcs.vat6 + calcs.vat12 + calcs.vat25).toFixed(2);
				calcs.RoundedSum =
